...
class FeedbackRepositoryTest implements BaseTest<Feedback> {
	...
	public void criteriaTest(ApplicationContext context) {
		// TODO Auto-generated method stub
		FeedbackDao feedbackDao = (FeedbackDao) context
				.getBean("feedbackRepository");
		UserDao userDao = (UserDao) context.getBean("userRepository");
		ProductCatalogUnitDao catalogUnitDao = (ProductCatalogUnitDao) context
				.getBean("productCatalogUnitRepository");

		User user = (User) userDao.findById(4);
		ProductCatalogUnit catalogUnit = (ProductCatalogUnit) catalogUnitDao
				.findById(1);
...
class ModuleRepositoryTest implements BaseTest<Module> {
	...
	public void criteriaTest(ApplicationContext context) {
		...
		ProductCatalogUnitDao productDao = (ProductCatalogUnitDao) context
				.getBean("productCatalogUnitRepository");
		ProviderDao providerDao = (ProviderDao) context
				.getBean("providerRepository");
		StorageModuleUnitDao moduleUnitDao = (StorageModuleUnitDao) context
				.getBean("storageModuleUnitRepository");
		ProductTemplateDao templateDao = (ProductTemplateDao) context
				.getBean("productTemplateRepository");
		ShipmentUnitDao shipmentUnitDao = (ShipmentUnitDao) context
				.getBean("shipmentUnitRepository");
		RequestUnitDao requestUnitDao = (RequestUnitDao) context
				.getBean("requestUnitRepository");
		...
		StorageModuleUnit moduleUnit = (StorageModuleUnit) moduleUnitDao
				.findById(2);
...
class OrderUnitRepositoryTest implements BaseTest<OrderUnit> {
	...
	public void criteriaTest(ApplicationContext context) {
		...
		OrderUnitDao orderUnitDao = (OrderUnitDao) context
				.getBean("orderUnitRepository");
...
class ProductCatalogUnitRepositoryTest implements BaseTest<ProductCatalogUnit> {
	...
	public void Show(ProductCatalogUnit entity) {
		out.println(entity.getId() + " " + entity.getName() + " "
				+ entity.getCost() + " " + entity.getCoefficient());
	}
...
class ProductTemplateRepositoryTest implements BaseTest<ProductTemplate> {
	...
	public void criteriaTest(ApplicationContext context) {
		...
		List<ProductTemplate> templates = templateDao
				.getProductTemplatesList(product);
		Show(templates);
...
class SaleRepositoryTest implements BaseTest<Sale> {
	...
	public void Show(Sale entity) {
		// TODO Auto-generated method stub
		out.println(entity.getId() + " " + entity.getStartTime() + " "
				+ entity.getEndTime());
	}
...
class StatusRepositoryTest implements BaseTest<Status> {
	...
	public void criteriaTest(ApplicationContext context) {
		// TODO Auto-generated method stub
		StatusDao statusDao = (StatusDao) context.getBean("statusRepository");
		OrderUnitDao unitDao = (OrderUnitDao) context
				.getBean("orderUnitRepository");
